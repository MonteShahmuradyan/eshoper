...
def cart(request):
    ...
    for i in cart_list:
        ...
        price_list.append(cart_list.single_price)
    return render(request, 'main/cart.html', context={
        'cart_list':cart_list,
        'summ':sum(price_list),
        'rast':int(sum(price_list) * 15 / 100),
        'total':sum(price_list) - int(sum(price_list) * 15 / 100),
        'mydict':quantity_list,
        'price_list':price_list,
        'full_list':zip(cart_list, price_list)
    })
...
def update_cart(request):
    ...
    if request.method == 'POST':
        prod_id = request.POST.get('id')
        prod_id_down = request.POST.get('id_down')
        if prod_id:
            quantity_list[int(prod_id)] += 1
            return redirect('cart')
        elif prod_id_down:
            if quantity_list[int(prod_id_down)] <= 0:
                return redirect('cart')
            else:
                quantity_list[int(prod_id_down)] -= 1
                return redirect('cart')
